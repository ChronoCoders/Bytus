...
import { CreditCard, Plus, Shield, Lock, Wifi, EyeOff, Eye } from "lucide-react";
  ...
  const toggleFreeze = (cardId: string = "default") => {
    setFrozenCards(prev => {
      ...
      toast({
        ...
        description: newState[cardId] ? "Your card is now temporarily locked." : "Your card is active and ready to use.",
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your physical and virtual cards.</p>
          </div>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Choose your card type. Physical cards will be shipped to your registered address.
                </DialogDescription>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-4 py-4">
                ...
                <div className="border rounded-xl p-4 cursor-pointer hover:border-primary hover:bg-primary/5 transition-all text-center space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground">Shipped in 3-5 days</div>
                </div>
              </div>
              <DialogFooter>
                <Button onClick={() => toast({ title: "Card Ordered", description: "Your new card is on the way!" })}>
                  Order Card
                </Button>
              </DialogFooter>
            </DialogContent>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="relative aspect-[1.586/1] w-full max-w-md mx-auto perspective-1000 group">
              <div className={`w-full h-full rounded-2xl bg-linear-to-br from-gray-900 to-black p-6 sm:p-8 text-white shadow-2xl relative overflow-hidden transition-all duration-500 transform group-hover:scale-105 ${isFrozen ? 'grayscale opacity-80' : ''}`}>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-md mx-auto">
              ...
              >
                {pinVisible ? <Eye className="w-5 h-5" /> : <EyeOff className="w-5 h-5" />}
                <span className="text-xs">{pinVisible ? "1234" : "Show Pin"}</span>
                {pinVisible && (
              ...
              <Button variant="outline" className="flex flex-col items-center gap-2 h-auto py-4 border-border hover:bg-accent hover:text-primary">
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage how your card can be used.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {[
                  { id: "online", label: "Online Payments", desc: "Allow payments on websites and apps" },
                  { id: "atm", label: "ATM Withdrawals", desc: "Allow cash withdrawals at ATMs" },
                  { id: "international", label: "International Use", desc: "Allow payments outside your home country" },
                  { id: "contactless", label: "Contactless Payments", desc: "Tap to pay functionality" },
                ].map((setting) => (
                  <div key={setting.id} className="flex items-center justify-between space-x-2">
                    <Label htmlFor={setting.id} className="flex flex-col space-y-1">
                      ...
                      <span className="font-normal text-xs text-muted-foreground">{setting.desc}</span>
                    </Label>
                    <Switch 
                      id={setting.id} 
